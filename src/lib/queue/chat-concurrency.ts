 ...
 *   hanxing:chat:ent:<entId>:concurrent     企业在途流数（chatMaxConcurrent）
 *   hanxing:chat:model:<modelId>:concurrent 模型全局在途流数（maxConcurrent）
 *
 * 企业/模型上限 ≤0 视为不限；TTL 取模型 taskTimeout + 60s 缓冲：
 * 进程崩溃未 release 时计数器自动归零。
 */

const userChatKey = (userId: string) => `hanxing:chat:user:${userId}:concurrent`
const entChatKey = (enterpriseId: string) =>
  `hanxing:chat:ent:${enterpriseId}:concurrent`
...
local entMax = tonumber(ARGV[2])
if entMax > 0 then
  local entCur = tonumber(redis.call('GET', KEYS[2]) or '0')
  if entCur >= entMax then return 0 end
end
local modelMax = tonumber(ARGV[3])
if modelMax > 0 then
  local modelCur = tonumber(redis.call('GET', KEYS[3]) or '0')
...
/** acquire/release 共用的槽位 key 装配（上限 ≤0 的维度不计数） */
function slotKeys(opts: {
  userId: string
  enterpriseId: string
  modelId: string
  ...
  const keys = [userChatKey(opts.userId)]
  if (opts.enterpriseMaxConcurrent > 0)
    keys.push(entChatKey(opts.enterpriseId))
  if (opts.modelMaxConcurrent > 0) keys.push(modelChatKey(opts.modelId))
  return keys
}
...
export async function acquireChatSlot(opts: {
  ...
  enterpriseId: string
  modelId: string
  /** 企业对话并发上限（chatMaxConcurrent；≤0 = 不限） */
  enterpriseMaxConcurrent: number
  modelMaxConcurrent: number
  ttlSec: number
}): Promise<boolean> {
